...
#         -DHERMETIC_LIBC=gnu.2.28 [-DHERMETIC_LLVM_PACKAGE=ON] \
#         [-DHERMETIC_LLVM_RUNTIME_SANITIZERS=ON] -P runtimes/build_runtimes.cmake
#
# or the libc++ set for a Windows (MSVC ABI) target:
#
#   HERMETIC_ACCEPT_MICROSOFT_EULA=1 cmake -DHERMETIC_TARGET=windows-x86_64 \
#         [-DHERMETIC_MSVC_TOOLSET_VERSION=...] -P runtimes/build_runtimes.cmake
#
...
if(NOT HERMETIC_TARGET)
  message(FATAL_ERROR "HERMETIC_TARGET is required (e.g. linux-x86_64)")
endif()
...
hermetic_llvm_provide_compiler("${HERMETIC_LLVM_VERSION}" "${HERMETIC_HOST_OS}" "${HERMETIC_HOST_ARCH}" dist)
hermetic_log("Compiler: LLVM ${dist_VERSION} at ${dist_ROOT}")
hermetic_target_info("${HERMETIC_TARGET}" tgt)
if(tgt_OS STREQUAL "windows" AND HERMETIC_WINDOWS_ABI STREQUAL "gnu")
  hermetic_llvm_build_mingw_runtime_set("${dist_ROOT}" "${dist_VERSION}" "${HERMETIC_TARGET}" set_dir)
elseif(tgt_OS STREQUAL "windows")
  hermetic_provide_windows_sdk("${tgt_ARCH}" win)
  ...
  hermetic_llvm_build_windows_runtime_set("${dist_ROOT}" "${dist_VERSION}" "${HERMETIC_TARGET}" set_dir)
elseif(tgt_OS STREQUAL "wasm")
  hermetic_llvm_build_wasm_runtime_set("${dist_ROOT}" "${dist_VERSION}" "${HERMETIC_TARGET}" set_dir)
else()
  hermetic_llvm_build_runtime_set("${dist_ROOT}" "${dist_VERSION}" "${HERMETIC_TARGET}" "${HERMETIC_LIBC}" set_dir)
endif()
