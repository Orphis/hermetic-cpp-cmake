...
foreach(_hl_env HERMETIC_KEEP_ARCHIVES HERMETIC_KEEP_BUILD_DIRS HERMETIC_SHOW_PROGRESS HERMETIC_VERBOSE)
...
set(HERMETIC_HOST_EXE "")
if(CMAKE_HOST_WIN32)
  set(HERMETIC_HOST_EXE ".exe")
endif()
...
function(hermetic_host_executable PATH OUT)
  if(EXISTS "${PATH}${HERMETIC_HOST_EXE}")
    set(${OUT} "${PATH}${HERMETIC_HOST_EXE}" PARENT_SCOPE)
    return()
  endif()
...
# when building Windows targets reproducibly (see hermetic_configure).
set(HERMETIC_CACHE_LINK_NAME "hermetic-cpp")
...
# needed, unlike a symbolic link there). Symbolic links name TARGET
# relative to LINK's directory. An existing link to somewhere else
...
# a warning otherwise.
function(hermetic_link_directory TARGET LINK OUT_OK)
  set(${OUT_OK} FALSE PARENT_SCOPE)
  file(REAL_PATH "${TARGET}" target_real)
  get_filename_component(parent "${LINK}" DIRECTORY)
  file(MAKE_DIRECTORY "${parent}")
  if(NOT CMAKE_HOST_WIN32)
    # A relative target keeps the link inside the tree when the cache is in
    # it (remote execution uploads the build directory's inputs as a tree).
    file(REAL_PATH "${parent}" parent_real)
    file(RELATIVE_PATH target_rel "${parent_real}" "${target_real}")
  endif()
  if(EXISTS "${LINK}")
    file(REAL_PATH "${LINK}" link_real)
    set(link_text "")
    if(NOT CMAKE_HOST_WIN32 AND IS_SYMLINK "${LINK}")
      file(READ_SYMLINK "${LINK}" link_text)
    endif()
    if(link_real STREQUAL target_real AND (CMAKE_HOST_WIN32 OR link_text STREQUAL target_rel))
    ...
    if(EXISTS "${LINK}")
      message(WARNING "[hermetic-cpp] ${LINK} exists and is not a link to ${TARGET}; remove it")
      return()
    endif()
  endif()
  if(CMAKE_HOST_WIN32)
    ...
    file(CREATE_LINK "${target_rel}" "${LINK}" SYMBOLIC RESULT rc)
    set(err "${rc}")
  endif()
  if(EXISTS "${LINK}")
    file(REAL_PATH "${LINK}" link_real)
  ...
  message(WARNING "[hermetic-cpp] Could not link ${LINK} to ${TARGET}: ${err}")
endfunction()

function(hermetic_log)
  message(STATUS "[hermetic-cpp] ${ARGN}")
endfunction()

function(hermetic_debug)
  if(HERMETIC_VERBOSE)
    message(STATUS "[hermetic-cpp] ${ARGN}")
  endif()
endfunction()
...
function(hermetic_detect_host OUT_OS OUT_ARCH)
  ...
  elseif(os MATCHES "^Windows")
    ...
    hermetic_fatal("Unsupported host OS: ${os}")
  endif()
  ...
  elseif(arch STREQUAL "riscv64")
    ...
    hermetic_fatal("Unsupported host architecture: ${arch}")
  endif()
...
function(hermetic_detect_linux_distribution OUT_NAME OUT_VERSION)
  set(name "")
  set(version "")
...
# Resolves the on-disk cache directory into HERMETIC_CACHE_DIR.
function(hermetic_resolve_cache_dir)
  if(HERMETIC_CACHE_DIR)
    set(dir "${HERMETIC_CACHE_DIR}")
  elseif(DEFINED ENV{HERMETIC_CACHE_DIR} AND NOT "$ENV{HERMETIC_CACHE_DIR}" STREQUAL "")
    set(dir "$ENV{HERMETIC_CACHE_DIR}")
  elseif(CMAKE_HOST_WIN32)
    set(dir "$ENV{LOCALAPPDATA}/hermetic-cpp")
  elseif(DEFINED ENV{XDG_CACHE_HOME} AND NOT "$ENV{XDG_CACHE_HOME}" STREQUAL "")
    set(dir "$ENV{XDG_CACHE_HOME}/hermetic-cpp")
  else()
    set(dir "$ENV{HOME}/.cache/hermetic-cpp")
  endif()
  file(TO_CMAKE_PATH "${dir}" dir)
  get_filename_component(dir "${dir}" ABSOLUTE)
  set(HERMETIC_CACHE_DIR "${dir}" PARENT_SCOPE)
endfunction()

# Reads a JSON file into ${OUT_VAR}, failing with a useful message.
function(hermetic_read_json PATH OUT_VAR)
  if(NOT EXISTS "${PATH}")
    hermetic_fatal("JSON file not found: ${PATH}")
  endif()
  file(READ "${PATH}" content)
  string(JSON _type ERROR_VARIABLE err TYPE "${content}")
  if(err)
    hermetic_fatal("Failed to parse ${PATH}: ${err}")
  endif()
...
function(hermetic_fetch_archive)
  ...
  foreach(required NAME KIND URLS OUT_DIR)
    if(NOT A_${required})
      hermetic_fatal("hermetic_fetch_archive: missing ${required}")
    endif()
  endforeach()
  ...
  if(A_HASH)
    if(NOT A_HASH MATCHES "^([A-Za-z0-9]+)=([0-9a-fA-F]+)$")
      hermetic_fatal("hermetic_fetch_archive: HASH must be <ALGO>=<hex>, not '${A_HASH}'")
    endif()
  ...
  elseif(A_SHA256)
    ...
    hermetic_fatal("hermetic_fetch_archive: missing SHA256 or HASH")
  endif()

  set(dest "${HERMETIC_CACHE_DIR}/${A_KIND}/${A_NAME}")
  ...
  file(MAKE_DIRECTORY "${HERMETIC_CACHE_DIR}/locks" "${HERMETIC_CACHE_DIR}/downloads" "${HERMETIC_CACHE_DIR}/${A_KIND}")
  string(REPLACE "/" "-" lock_name "${A_KIND}-${A_NAME}")
  set(lock "${HERMETIC_CACHE_DIR}/locks/${lock_name}.lock")
  file(LOCK "${lock}" GUARD FUNCTION TIMEOUT 7200 RESULT_VARIABLE lock_result)
  if(NOT lock_result EQUAL 0)
    hermetic_fatal("Could not acquire ${lock}: ${lock_result}")
  endif()
  ...
  set(archive "${HERMETIC_CACHE_DIR}/downloads/${basename}")

  if(EXISTS "${archive}")
    file(${A_ALGO} "${archive}" actual)
    if(NOT actual STREQUAL A_SHA256)
      hermetic_log("Discarding ${archive}: checksum mismatch")
      file(REMOVE "${archive}")
    else()
      hermetic_log("Using cached archive ${archive}")
    endif()
  endif()
  ...
  if(NOT EXISTS "${archive}")
    ...
    set(progress "")
    if(HERMETIC_SHOW_PROGRESS)
      set(progress SHOW_PROGRESS)
    endif()
    if(NOT DEFINED HERMETIC_DOWNLOAD_ATTEMPTS)
      set(HERMETIC_DOWNLOAD_ATTEMPTS 3)
    endif()
    foreach(url IN LISTS A_URLS)
      foreach(attempt RANGE 1 ${HERMETIC_DOWNLOAD_ATTEMPTS})
        ...
        message(WARNING "[hermetic-cpp] Download of ${url} failed (attempt ${attempt}/${HERMETIC_DOWNLOAD_ATTEMPTS}): ${text}")
      endforeach()
      if(downloaded)
        break()
      endif()
    ...
    file(REMOVE "${archive}.part")
    if(NOT downloaded)
      hermetic_fatal("Could not download ${A_NAME} from any of: ${A_URLS}")
    endif()
  endif()

  hermetic_log("Extracting ${basename} into ${dest}")
  set(tmp "${dest}.tmp")
  ...
  if(A_PKG_PREFIX)
    ...
    if(NOT rc EQUAL 0)
      ...
      hermetic_fatal("Could not expand ${basename} with pkgutil: ${err}${out}${hint}")
    endif()
    set(A_STRIP_COMPONENTS 0)
  else()
    set(patterns "")
  ...
  while(remaining GREATER 0)
    ...
    if(NOT count EQUAL 1)
      hermetic_fatal("Cannot strip ${A_STRIP_COMPONENTS} leading path component(s) from ${basename}: ${current} has ${count} entries (${entries})")
    endif()
    list(GET entries 0 current)
    if(NOT IS_DIRECTORY "${current}")
      hermetic_fatal("Cannot strip path components from ${basename}: ${current} is not a directory")
    endif()
...
# caller's scope.
function(hermetic_append_flags VAR)
  set(value "${${VAR}}")
